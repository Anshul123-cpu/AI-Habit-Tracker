...
export const weeklyReport = async (req, res) => {
  try {
    ...
    if (!ctx.perHabit.length) {
      return res.json({
        content:
          "You don't have any active habits yet. Create your first habit to start tracking — I’ll generate a weekly report once there’s data.",
      });
    }
...
export const testGemini = async (req, res) => {
  try {
    const { ok, content } = await chatCompletion({
      ...
      user: "Reply with exactly: Gemini is working",
      temperature: 0.2,
    });
